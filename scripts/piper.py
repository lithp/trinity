...
def traverse_branch(should_return, db, depth, path, branch, start):
    if branch[16] != b'':
    ...
    for i in range(start_index, 16):
        if branch[i] != b'':
            yield from traverse_node(should_return, db, depth, path + (i,), branch[i], start[1:])
...
def traverse_node(should_return, db, depth, path, nodehash, start):
    node_rlp, node = get_node(db, nodehash)
    node_type = get_node_type(node)
    if node_type == NODE_TYPE_BRANCH:
        obj = Node(depth, 'branch', node_rlp, path)
        if should_return(obj):
            yield obj
            yield from traverse_branch(should_return, db, depth + 1, path, node, start)
    elif node_type == NODE_TYPE_LEAF:
        rest = extract_key(node)
        # TODO: also traverse the state root?
        yield Node(depth, 'leaf', node_rlp, path + rest)
    elif node_type == NODE_TYPE_EXTENSION:
        # TODO: decide whether to yield this node, does it still match {start}
        # TODO: test that we're building this path correctly
        rest = extract_key(node)
        full_path = path + rest
        obj = Node(depth, 'extension', node_rlp, full_path)
        if should_return(obj):
            yield obj
            yield from traverse_node(should_return, db, depth+1, full_path, node[1], start[1:])
    else:
...
def traverse_100_each(db, root):
    counts = [0] * 7
    def should_return(node):
        counts[node.depth] += 1
        if counts[node.depth] > 100:
#            print(f'depth={node.depth} seen={counts[node.depth]} not returning')
            return False
#        print(f'depth={node.depth} seen={counts[node.depth]} returning')
        return True

    count = 0
    for node in traverse_node(should_return, db, 0, tuple(), root, [0]):
        yield node
        count += 1
        if count >= 100:
            return
...
def export_them(args):
    db = open_db(args.db)
    root = state_root_for(args, db)

    for node in traverse_100_each(db, root):
        print(f"depth={node.depth} rlp={node.rlp.hex()}")
